	...
	grunt.registerTask('qunit', 'Run the test suite on PhantomJS', function () {
		...
		phantomjs.spawn('http://localhost:8000/test/index.html', {
			...
			// Once phantomjs is halted
				...
				expected = normalize(String(expected).trim());
				actual = normalize(String(actual).trim());
				...
				if ( expected === actual ) {
					...
					grunt.log.writeln('XML output does not match expected pattern.');
					grunt.log.writeln('Expected:\n' + expected + '\nActual:\n' + actual);
					grunt.fail.warn('XML output does not match expected pattern.');
					done(false);
				}
			}
		});
